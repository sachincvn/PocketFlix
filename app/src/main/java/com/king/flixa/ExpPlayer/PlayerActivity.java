...
import com.king.flixa.dtpv.DoubleTapPlayerView;
import com.king.flixa.dtpv.youtube.YouTubeOverlay;
...
public class PlayerActivity extends AppCompatActivity {
    ...
    public static final String VIDEO_REFERER = "referer";
    public static final String VIDEO_COOKIE = "cookie";
    public static final String VIDEO_USER_AGENT = "user-agent";
    public static final String VIDEO_ORIGIN = "origin";
    public static final String VIDEO_URL = "stream-url";
    public static final String VIDEO_LICENSE = "license";
    public static final String VIDEO_TITLE = "title";
    public static final String VIDEO_SCHEME = "drmScheme";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        buttonOpen.setImageResource(R.drawable.ic_quality);
...
//        });
    ...
    public void initializePlayer() {
        ...
        int videoScheme = intent.getStringExtra(VIDEO_SCHEME) != null ? Integer.parseInt(intent.getStringExtra(VIDEO_SCHEME)) : 0;
        UUID selectedUuid = videoScheme == 2 ? C.CLEARKEY_UUID : C.WIDEVINE_UUID;

        videoUrl = "https://hiskidxd.com/T5/JIOTV/app/live.php?id=291&e=.m3u8?|Referer=https://hiskidxd.com/";
        ...
        if (videoUrl!=null&&videoUrl.contains("|")){
           VideoModel videoModel = Utilities.getUrlParameters(videoUrl);
           videoUrl = videoModel.streamUrl;
           userAgentValue = videoModel.userAgent;
           refererValue = videoModel.refererValue;
           cookieValue = videoModel.cookieValue;
           originValue = videoModel.originValue;
        }
        else{
            userAgentValue = intent.getStringExtra(VIDEO_USER_AGENT);
            refererValue = intent.getStringExtra(VIDEO_REFERER);
            cookieValue = intent.getStringExtra(VIDEO_COOKIE);
            originValue = intent.getStringExtra(VIDEO_ORIGIN);
        }
        ...
        if(userAgentValue!=null && !userAgentValue.isEmpty()){
            ...
            headers.put("User-Agent","plaYtv/6.0.9 (Linux; Android 5.1.1) ExoPlayerLib/2.13.2");
        }
        ...
        boolean isNetworkUri =  true; //= mPrefs.mediaUri != null && Utils.isSupportedNetworkUri(mPrefs.mediaUri);
        haveMedia = true;// = mPrefs.mediaUri != null && (Utils.fileExists(this, mPrefs.mediaUri) || isNetworkUri);
        ...
        @SuppressLint("WrongConstant") RenderersFactory renderersFactory = new DefaultRenderersFactory(this)
                .setExtensionRendererMode(mPrefs.decoderPriority).forceEnableMediaCodecAsynchronousQueueing();
        ...
        DefaultMediaSourceFactory mediaSourceFactory = new DefaultMediaSourceFactory(defaultHttpDataSourceFactory, extractorsFactory);
        MediaItem.DrmConfiguration drmConfiguration = null;
        if(selectedUuid == C.CLEARKEY_UUID){
            String clearKey = intent.getStringExtra((VIDEO_LICENSE));
            if (!isValidJson(clearKey)) {
                String[] keyData = clearKey.split(":");
                if (keyData.length >= 2) {
                    clearKey = clearKeyHexToBase64Json(keyData[0],keyData[1]);
                }
            }
        ...
        } else if (selectedUuid == C.WIDEVINE_UUID) {
            drmConfiguration = new MediaItem.DrmConfiguration.Builder(selectedUuid)
                    .setScheme(selectedUuid)
                    .setLicenseUri(intent.getStringExtra(VIDEO_LICENSE))
        ...
        if (haveMedia) {
            ...
            titleView.setText(intent.getStringExtra(VIDEO_TITLE));
...
//            Utils.markChapters(this, mPrefs.mediaUri, controlView);
    ...
    public  static  String clearKeyHexToBase64Json(String keyIdHex,String keyHex){
        // Convert Key ID and Key from Hex to Base64
        String keyIdBase64 = convertHexToBase64(keyIdHex);
        String keyBase64 = convertHexToBase64(keyHex);

        // Create the ClearKeys DRM JSON
        String json = "{\n" +
                "    \"keys\":[\n" +
                "       {\n" +
                "          \"kty\":\"oct\",\n" +
    ...
    public static String convertHexToBase64(String hex) {
        ...
        return base64.replace("=", ""); // Remove trailing '=' characters
    }
...
//                        switched = Utils.switchFrameRate(PlayerActivity.this, mPrefs.mediaUri, play);
    ...
    private void updateLoading(final boolean enableLoading) {
        if (enableLoading) {
            ...
            playerMessage.setVisibility(View.GONE);
